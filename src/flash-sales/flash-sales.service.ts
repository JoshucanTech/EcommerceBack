...
export class FlashSalesService {
  ...
  async create(createFlashSaleDto: CreateFlashSaleDto, userId: string) {
    const { name, description, startDate, endDate, isActive, items } = createFlashSaleDto
    ...
    if (start < new Date() && !isActive) {
      throw new BadRequestException("Cannot create an inactive flash sale with a past start date")
    }
    ...
    if (products.length !== productIds.length) {
      throw new BadRequestException("Some products do not exist or do not belong to this vendor")
    }
    ...
    for (const item of items) {
      ...
      if (!product.inventory || product.inventory.quantity < item.quantity) {
        throw new BadRequestException(`Product ${product.name} has insufficient inventory`)
      }
    }
  ...
  async findAll(params: {
    ...
    const processedFlashSales = flashSales.map((flashSale) => {
      const now = new Date()
      const isActive = flashSale.isActive && flashSale.startDate <= now && flashSale.endDate >= now

      return {
        ...flashSale,
        itemCount: flashSale._count.items,
        status: isActive ? "ACTIVE" : flashSale.startDate > now ? "UPCOMING" : "EXPIRED",
        _count: undefined,
      }
    })
  ...
  async findOne(idOrSlug: string) {
    ...
    if (!flashSale) {
      throw new NotFoundException(`Flash sale with ID or slug ${idOrSlug} not found`)
    }
    ...
    const items = flashSale.items.map((item) => {
      ...
      return {
        ...
        product: {
          ...
          availableQuantity: Math.min(item.quantity - item.soldCount, item.product.inventory?.quantity || 0),
        },
      }
    })
    ...
    const now = new Date()
    const status =
      flashSale.isActive && flashSale.startDate <= now && flashSale.endDate >= now
        ? "ACTIVE"
  ...
  async update(id: string, updateFlashSaleDto: UpdateFlashSaleDto, user: any) {
    ...
    if (user.role !== "ADMIN" && flashSale.vendor.userId !== user.id) {
      throw new ForbiddenException("You do not have permission to update this flash sale")
    }

    // Extract data from DTO
    const { name, description, startDate, endDate, isActive, items } = updateFlashSaleDto
  ...
  async remove(id: string, user: any) {
    ...
    // Check if user is the vendor of the flash sale or an admin
    if (user.role !== "ADMIN" && flashSale.vendor.userId !== user.id) {
      throw new ForbiddenException("You do not have permission to delete this flash sale")
    }
