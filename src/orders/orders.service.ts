...
export class OrdersService {
  ...
  async create(createOrderDto: CreateOrderDto, userId: string) {
    const { items, paymentMethod, addressId, couponCode, notes } = createOrderDto
    ...
    if (products.length !== productIds.length) {
      throw new BadRequestException("Some products do not exist or are not published")
    }
    ...
    for (const vendorId in itemsByVendor) {
      ...
      for (const { product, quantity } of vendorItems) {
        ...
        const activeFlashSale = product.flashSaleItems.find((fsi) => fsi.flashSale !== null)
        const basePrice = product.discountPrice || product.price
        const itemPrice = activeFlashSale ? basePrice * (1 - activeFlashSale.discountPercentage / 100) : basePrice
  ...
  async findOne(id: string, user: any) {
    ...
    if (user.role !== "ADMIN" && order.userId !== user.id && order.vendor?.userId !== user.id) {
  ...
  async updateStatus(id: string, updateOrderStatusDto: UpdateOrderStatusDto, user: any) {
    ...
    if (user.role !== "ADMIN" && (user.role !== "VENDOR" || order.vendor?.userId !== user.id)) {
