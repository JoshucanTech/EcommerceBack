...
export class DeliveriesController {
  ...
  @ApiQuery({ name: "limit", required: false, type: Number, description: "Items per page" })
  @ApiQuery({ name: "status", required: false, type: String, description: "Filter by status" })
  @ApiResponse({ status: 200, description: "Returns paginated rider deliveries" })
  @ApiResponse({ status: 401, description: "Unauthorized" })
  @ApiResponse({ status: 403, description: "Forbidden - Insufficient permissions" })
  findRiderDeliveries(
    @CurrentUser() user,
  ...
  @ApiQuery({ name: "limit", required: false, type: Number, description: "Items per page" })
  @ApiQuery({ name: "status", required: false, type: String, description: "Filter by status" })
  @ApiResponse({ status: 200, description: "Returns paginated vendor deliveries" })
  @ApiResponse({ status: 401, description: "Unauthorized" })
  @ApiResponse({ status: 403, description: "Forbidden - Insufficient permissions" })
  ...
  @ApiQuery({ name: "limit", required: false, type: Number, description: "Items per page" })
  @ApiQuery({ name: "status", required: false, type: String, description: "Filter by status" })
  @ApiResponse({ status: 200, description: "Returns paginated user deliveries" })
  @ApiResponse({ status: 401, description: "Unauthorized" })
  findUserDeliveries(
    @CurrentUser() user,
    @Query('page') page?: number,
  ...
  @ApiResponse({ status: 401, description: "Unauthorized" })
  @ApiResponse({ status: 403, description: "Forbidden - Not authorized to view this delivery" })
  ...
  @ApiResponse({ status: 400, description: "Bad request" })
  @ApiResponse({ status: 401, description: "Unauthorized" })
  @ApiResponse({ status: 403, description: "Forbidden - Not authorized to update this delivery" })
  @ApiResponse({ status: 404, description: "Delivery not found" })
  updateStatus(@Param('id') id: string, @Body() updateDeliveryStatusDto: UpdateDeliveryStatusDto, @CurrentUser() user) {
    return this.deliveriesService.updateStatus(id, updateDeliveryStatusDto, user)
  }
}
