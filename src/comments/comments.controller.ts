...
export class CommentsController {
  ...
  @ApiResponse({ status: 200, description: "Returns paginated comments for the product" })
  ...
  @ApiQuery({ name: "page", required: false, type: Number, description: "Page number" })
  @ApiQuery({ name: "limit", required: false, type: Number, description: "Items per page" })
  @ApiResponse({ status: 200, description: "Returns paginated replies to the comment" })
  @ApiResponse({ status: 404, description: "Comment not found" })
  findReplies(@Param('id') id: string, @Query('page') page?: number, @Query('limit') limit?: number) {
  ...
  @ApiResponse({ status: 404, description: "Comment not found" })
  update(@Param('id') id: string, @Body() updateCommentDto: UpdateCommentDto, @CurrentUser() user) {
  ...
  @ApiResponse({ status: 401, description: "Unauthorized" })
  @ApiResponse({ status: 403, description: "Forbidden - Not the comment owner or admin" })
  @ApiResponse({ status: 404, description: "Comment not found" })
