...
export class ReviewsController {
  ...
  @ApiResponse({ status: 200, description: "Returns paginated reviews for the product" })
  @ApiResponse({ status: 404, description: "Product not found" })
  ...
  @ApiQuery({ name: "page", required: false, type: Number, description: "Page number" })
  @ApiQuery({ name: "limit", required: false, type: Number, description: "Items per page" })
  @ApiResponse({ status: 200, description: "Returns paginated reviews by the user" })
  @ApiResponse({ status: 401, description: "Unauthorized" })
  findByUser(@CurrentUser() user, @Query('page') page?: number, @Query('limit') limit?: number) {
  ...
  @ApiResponse({ status: 404, description: "Review not found" })
  update(@Param('id') id: string, @Body() updateReviewDto: UpdateReviewDto, @CurrentUser() user) {
  ...
  @ApiResponse({ status: 401, description: "Unauthorized" })
  @ApiResponse({ status: 403, description: "Forbidden - Not the review owner or admin" })
  @ApiResponse({ status: 404, description: "Review not found" })
